public class Matrix {
    ...
    public int getValueAt(int rowPosition, int colPosition) throws IndexOutOfBoundsException {
        boolean flag = true;
		if (rowPosition >= this.nRow || rowPosition < 0) flag = false;
        if (colPosition >= this.nColumn || colPosition < 0) flag = false;
        if (!flag) {
    ...
    public Matrix add(Matrix other) throws InvalidOperationException {
        boolean flag = true;
        if (this.nRow != other.getNRow()) flag = false;
        if (this.nColumn != other.getNColumn()) flag = false;
        if (!flag) {
